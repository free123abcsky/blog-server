...
class Email extends Service {
  ...
  sendActiveMail (user) {
    ...
    const token = this.service.auth.genToken(user._id)
    const {appName} = this.config.transporter
    const html = `
      <p>您好：</p>
      <p>我们收到您在 ${appName} 的注册信息，请点击下面的链接来激活帐户：</p>
      <a href="/users/${user._id}/verify?confirm_token=${token}">激活链接</a>
      <p>若您没有在${appName}填写过注册信息，说明有人滥用了您的电子邮箱，请删除此邮件，我们对给您造成的打扰感到抱歉。</p>
      <p>${appName}谨上。</p>
    `
    return this.sent(user.email, appName + '帐号激活', html)
  }
